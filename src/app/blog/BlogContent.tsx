...
export default function BlogContent({ featured, rest }: { featured: Post | undefined; rest: Post[] }) {
  const [activeCategory, setActiveCategory] = useState("Toate");

  const filteredRest =
    activeCategory === "Toate" ? rest : rest.filter((p) => p.category === activeCategory);

  const filteredFeatured =
    activeCategory === "Toate" || featured?.category === activeCategory ? featured : undefined;
  ...
  return (
    <section className="py-20 bg-[#EEF2FF]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Featured post */}
        {filteredFeatured && (
          <Link href={`/blog/${filteredFeatured.slug}`} className="block mb-10 group">
                ...
                <div className="p-8 flex flex-col justify-center">
                  ...
                  <div className="flex items-center gap-4 text-xs text-[#6B7A9A]" style={{ fontFamily: "var(--font-body)" }}>
                    <div className="flex items-center gap-1">
                      <Clock size={12} />
                      {filteredFeatured.readTime} de lectură
                    </div>
        ...
        {filteredRest.length === 0 && !filteredFeatured && (
          <p className="text-center text-[#6B7A9A] py-16" style={{ fontFamily: "var(--font-body)" }}>
